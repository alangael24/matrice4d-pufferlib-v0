...
__global__ void mask_actor_obs_kernel(
        ...
        int rows, int obs_dim, int mask_prefix, int mask_target) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int total = rows * obs_dim;
    if (idx >= total) return;
    int col = idx % obs_dim;
    bool masked = col < mask_prefix;
    masked = masked || (mask_target && col >= 10 && col < 19);
...
PrecisionTensor make_actor_obs(PrecisionTensor& dst, PrecisionTensor& src,
    ...
    int rows = numel(src.shape) / src.shape[ndim(src.shape) - 1];
    int obs_dim = src.shape[ndim(src.shape) - 1];
...
extern "C" void net_callback_wrapper(void* ctx, int buf, int t) {
    ...
    PrecisionTensor actor_obs = obs_dst;
    if (hypers.actor_obs_mask_prefix > 0 || hypers.actor_obs_mask_target > 0.0f) {
        actor_obs = make_actor_obs(
            pufferl->rollout_actor_obs[buf], obs_dst,
            hypers.actor_obs_mask_prefix,
            hypers.actor_obs_mask_target > 0.0f ? 1 : 0,
            stream);
    }
...
void train_impl(PuffeRL& pufferl) {
    ...
    for (int mb = 0; mb < total_minibatches; ++mb) {
        ...
        if (pufferl.train_captured) {
            ...
            PrecisionTensor actor_obs_puf = obs_puf;
            if (hypers.actor_obs_mask_prefix > 0 || hypers.actor_obs_mask_target > 0.0f) {
                actor_obs_puf = make_actor_obs(
                    pufferl.train_actor_obs, obs_puf,
                    hypers.actor_obs_mask_prefix,
                    hypers.actor_obs_mask_target > 0.0f ? 1 : 0,
                    stream);
            }
...
std::unique_ptr<PuffeRL> create_pufferl_impl(HypersT& hypers,
    ...
    if (actor_obs_masking) {
        pufferl->train_actor_obs = {.shape = {minibatch_segments, horizon, input_size}};
        alloc_register(acts, &pufferl->train_actor_obs);
    }
