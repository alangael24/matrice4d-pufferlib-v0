...
typedef struct {
    ...
    float motor_thrust_scale[4]; // per-motor thrust multiplier relative to k_thrust
    float motor_tau[4]; // per-motor RPM time constant, seconds
    float yaw_torque_scale[4]; // per-motor yaw torque multiplier
...
typedef struct {
    ...
    float com_xy;
    float com_z;
    float authority_gated;
    float normalized_thrust_max;
    float usable_t2w_min;
    float usable_t2w_max;
    float mass_min;
    float mass_max;
    float inertia_min;
    float inertia_max;
    ...
    float motor_tau_min;
    float motor_tau_max;
    float yaw_torque_min;
    float yaw_torque_max;
    float linear_drag_min;
    float linear_drag_max;
    float angular_damping_min;
    float angular_damping_max;
    float profile_mix;
} DomainRandomization;
...
static inline bool dr_authority_gated(const DomainRandomization* dr) {
    return dr != NULL && dr->enabled > 0.0f && dr->authority_gated > 0.0f;
}

static inline bool dr_v1a2_low_authority_mix(const DomainRandomization* dr) {
    return dr_authority_gated(dr) && dr->profile_mix > 0.0f && dr->profile_mix < 2.0f;
}

static inline bool dr_edgefix_capped_mix(const DomainRandomization* dr) {
    return dr_authority_gated(dr) && dr->profile_mix >= 2.0f && dr->profile_mix < 3.0f;
}

static inline bool dr_structured_legacy_mix(const DomainRandomization* dr) {
    return dr_authority_gated(dr) && dr->profile_mix >= 3.0f && dr->profile_mix < 4.0f;
}

static inline float dr_sample_range(unsigned int* rng, float lo, float hi, float fallback) {
    if (hi <= lo) return fallback;
    return rndf(lo, hi, rng);
}

static inline int dr_risk_score(float usable_t2w, float mass_mult, float inertia_max,
                                float motor_thrust_min, float motor_tau_max,
...
static inline float motor_thrust_coeff(const Params* p, int i) {
    return p->k_thrust * p->motor_thrust_scale[i];
}

static inline float max_motor_thrust_i(const Params* p, int i) {
    return motor_thrust_coeff(p, i) * p->max_rpm * p->max_rpm;
}

static inline float total_max_motor_thrust(const Params* p) {
    float total = 0.0f;
    for (int i = 0; i < 4; i++) total += max_motor_thrust_i(p, i);
    return total;
}

static inline float max_motor_thrust(const Params* p) {
    return total_max_motor_thrust(p) * 0.25f;
}
...
static inline bool solve_allocation(const Params* p, float total_thrust, Vec3 torque, float out[4]) {
    ...
    for (int i = 0; i < 4; i++) {
        out[i] = clampf(a[i][4], 0.0f, max_motor_thrust_i(p, i));
    }
    return true;
}
...
static inline float thrust_to_rpm_i(const Params* p, int i, float thrust) {
    float k = fmaxf(motor_thrust_coeff(p, i), 1e-12f);
    thrust = clampf(thrust, 0.0f, max_motor_thrust_i(p, i));
    return sqrtf(thrust / k);
}

static inline float thrust_to_rpm(const Params* p, float thrust) {
    return thrust_to_rpm_i(p, 0, thrust);
}
...
static inline void init_drone(Drone* drone, unsigned int* rng, const DomainRandomization* dr) {
    bool authority_gated = dr_authority_gated(dr);
    float usable_t2w_min = dr != NULL ? dr->usable_t2w_min : 0.0f;
    float usable_t2w_max = dr != NULL ? dr->usable_t2w_max : 0.0f;
    float mass_min = dr != NULL ? dr->mass_min : 1.0f;
    float mass_max = dr != NULL ? dr->mass_max : 1.0f;
    float inertia_min = dr != NULL ? dr->inertia_min : 1.0f;
    float inertia_max = dr != NULL ? dr->inertia_max : 1.0f;
    float motor_thrust_min = dr != NULL ? dr->motor_thrust_min : 1.0f;
    float motor_thrust_max = dr != NULL ? dr->motor_thrust_max : 1.0f;
    float motor_tau_min = dr != NULL ? dr->motor_tau_min : BASE_K_MOT;
    float motor_tau_max = dr != NULL ? dr->motor_tau_max : BASE_K_MOT;
    float yaw_torque_min = dr != NULL ? dr->yaw_torque_min : 1.0f;
    float yaw_torque_max = dr != NULL ? dr->yaw_torque_max : 1.0f;
    float linear_drag_min = dr != NULL ? dr->linear_drag_min : 1.0f;
    float linear_drag_max = dr != NULL ? dr->linear_drag_max : 1.0f;
    float angular_damping_min = dr != NULL ? dr->angular_damping_min : 1.0f;
    float angular_damping_max = dr != NULL ? dr->angular_damping_max : 1.0f;
    float com_xy_range = (dr != NULL && dr->enabled > 0.0f) ? fabsf(dr->com_xy) : 0.0f;
    float com_z_range = (dr != NULL && dr->enabled > 0.0f) ? fabsf(dr->com_z) : 0.0f;
    float normalized_thrust_max = dr != NULL ? dr->normalized_thrust_max : 1.0f;
    ...
    if (dr_structured_legacy_mix(dr)) {
        float profile = rndf(0.0f, 1.0f, rng);
        if (profile < 0.35f) {
            float hard = rndf(0.0f, 1.0f, rng);
            if (hard < 0.65f) {
    ...
    } else if (dr_edgefix_capped_mix(dr)) {
        float profile = rndf(0.0f, 1.0f, rng);
        if (profile < 0.50f) {
            ...
            normalized_thrust_max = rndf(0.45f, 0.60f, rng);
    ...
    } else if (dr_v1a2_low_authority_mix(dr)) {
        float profile = rndf(0.0f, 1.0f, rng);
        if (profile < 0.40f) {
    ...
    for (int i = 0; i < 4; i++) {
        motor_tau[i] = BASE_K_MOT * motor_lag_mult;
    }
    float com_x = 0.0f;
    float com_y = 0.0f;
    float com_z = 0.0f;
    if (authority_gated) {
        float usable_t2w = 2.5f;
        for (int attempt = 0; attempt < 16; attempt++) {
            mass_mult = dr_sample_range(rng, mass_min, mass_max, 1.0f);
            ixx_mult = dr_sample_range(rng, inertia_min, inertia_max, 1.0f);
            iyy_mult = dr_sample_range(rng, inertia_min, inertia_max, 1.0f);
            izz_mult = dr_sample_range(rng, inertia_min, inertia_max, 1.0f);
            linear_drag_mult = dr_sample_range(rng, linear_drag_min, linear_drag_max, 1.0f);
            angular_damping_mult = dr_sample_range(
                rng, angular_damping_min, angular_damping_max, 1.0f);
            usable_t2w = dr_sample_range(rng, usable_t2w_min, usable_t2w_max, 2.5f);
            com_x = rndf(-com_xy_range, com_xy_range, rng);
            com_y = rndf(-com_xy_range, com_xy_range, rng);
            com_z = rndf(-com_z_range, com_z_range, rng);

            float motor_scale_min = 2.0f;
            float motor_tau_sample_max = 0.0f;
            for (int i = 0; i < 4; i++) {
                motor_thrust_scale[i] =
                    dr_sample_range(rng, motor_thrust_min, motor_thrust_max, 1.0f);
                motor_tau[i] = dr_sample_range(rng, motor_tau_min, motor_tau_max, BASE_K_MOT);
                yaw_torque_scale[i] =
                    dr_sample_range(rng, yaw_torque_min, yaw_torque_max, 1.0f);
                if (motor_thrust_scale[i] < motor_scale_min) motor_scale_min = motor_thrust_scale[i];
                if (motor_tau[i] > motor_tau_sample_max) motor_tau_sample_max = motor_tau[i];
            }

            if (force_three_plus_one) {
                int weak = (int)floorf(rndf(0.0f, 4.0f, rng));
                if (weak > 3) weak = 3;
                for (int i = 0; i < 4; i++) {
                    motor_thrust_scale[i] = dr_sample_range(rng, 0.96f, 1.06f, 1.0f);
                    motor_tau[i] = dr_sample_range(rng, 0.08f, 0.16f, BASE_K_MOT);
                    yaw_torque_scale[i] = dr_sample_range(rng, 0.90f, 1.10f, 1.0f);
                }
                motor_thrust_scale[weak] = dr_sample_range(rng, 0.78f, 0.88f, 0.84f);
                motor_tau[weak] = dr_sample_range(rng, 0.22f, 0.28f, 0.24f);
                yaw_torque_scale[weak] = dr_sample_range(rng, 0.70f, 0.95f, 0.85f);
            }

            if (force_fast_slow_mismatch) {
                int slow = (int)floorf(rndf(0.0f, 4.0f, rng));
                if (slow > 3) slow = 3;
                int fast = (slow + 2) & 3;
                motor_tau[slow] = dr_sample_range(rng, 0.20f, 0.26f, 0.22f);
                motor_tau[fast] = dr_sample_range(rng, 0.05f, 0.09f, 0.07f);
                motor_thrust_scale[slow] = dr_sample_range(rng, 0.90f, 1.05f, 0.98f);
                motor_thrust_scale[fast] = dr_sample_range(rng, 0.95f, 1.12f, 1.02f);
            }
        ...
        if (direct_k_thrust_profile) {
            k_thrust_mult = dr_sample_range(
                rng, direct_k_thrust_min, direct_k_thrust_max, 2.91f);
        } else {
            float cap = clampf(normalized_thrust_max, 1e-3f, 1.0f);
        ...
        mass_mult = dr_sample_mult(rng, dr_param_range(dr, dr == NULL ? 0.0f : dr->mass));
        ixx_mult = dr_sample_mult(rng, dr_param_range(dr, dr == NULL ? 0.0f : dr->inertia));
        iyy_mult = dr_sample_mult(rng, dr_param_range(dr, dr == NULL ? 0.0f : dr->inertia));
        izz_mult = dr_sample_mult(rng, dr_param_range(dr, dr == NULL ? 0.0f : dr->inertia));
        linear_drag_mult = dr_sample_mult(rng, dr_param_range(dr, dr == NULL ? 0.0f : dr->linear_drag));
        com_x = rndf(-com_xy_range, com_xy_range, rng);
        com_y = rndf(-com_xy_range, com_xy_range, rng);
        com_z = rndf(-com_z_range, com_z_range, rng);
    }
    ...
    drone->params.k_ang_damp = BASE_K_ANG_DAMP * angular_damping_mult;
    ...
    drone->params.normalized_thrust_max = clampf(normalized_thrust_max, 0.0f, 1.0f);
    ...
    for (int i = 0; i < 4; i++) {
        drone->params.motor_thrust_scale[i] = motor_thrust_scale[i];
        drone->params.motor_tau[i] = motor_tau[i];
        drone->params.yaw_torque_scale[i] = yaw_torque_scale[i];
    }
    ...
    for (int i = 0; i < 4; i++)
        drone->state.rpms[i] = thrust_to_rpm_i(&drone->params, i, trim[i]);
...
static inline void compute_derivatives(State* state, Params* params, float* actions,
    ...
    for (int i = 0; i < 4; i++) {
        float target_thrust;
        float max_thrust_i = max_motor_thrust_i(params, i);
        if (params->action_mode == M4D_ACTION_NORMALIZED_THRUST) {
            target_thrust = action_to_target_thrust(params, actions[i], trim[i], max_thrust_i);
        } else {
        ...
        target_rpms[i] = thrust_to_rpm_i(params, i, target_thrust);
    }

    float rpm_dot[4];
    for (int i = 0; i < 4; i++) {
        rpm_dot[i] = (1.0f / fmaxf(params->motor_tau[i], 1e-4f))
                   * (target_rpms[i] - state->rpms[i]);
    ...
    for (int i = 0; i < 4; i++) {
        ...
        T[i] = motor_thrust_coeff(params, i) * rpm * rpm;
    }
    ...
    for (int i = 0; i < 4; i++) {
        ...
        Tau_prop.z += params->k_drag * params->yaw_torque_scale[i] * params->yaw_sign[i] * T[i];
    }
