...
# Stage 1: single-segment Swift-like racing with gate-mask vision and a
# privileged critic. The actor sees proprioception + 16x16 gate-mask pixels;
# the critic can still use the target/proprio fields for value learning.
#
# Default is from scratch because the linear-race baseline transfers poorly to
# the first Swift-like gate. Set FROM_SCRATCH=0 BASE=/path/to/checkpoint.bin to
# fine-tune from an existing policy.

if [[ "${FROM_SCRATCH:-1}" == "1" ]]; then
  BASE="${BASE:-}"
elif [[ -z "${BASE:-}" ]]; then
...
export BATCH_ID="${BATCH_ID:-v0_minimal_active_vision_16x16_race_stage1_segments_gatemask_privcritic_scratch_seed44_500m}"
