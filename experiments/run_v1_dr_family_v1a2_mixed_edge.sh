...
BATCH_ID="${BATCH_ID:-v1_norm_thrust_dr_family_v1a2_mixed_edge_$(date -u +%Y%m%dT%H%M%SZ)}"
...
ALPHA_ACTION_DELTA="${ALPHA_ACTION_DELTA:-0.02}"
ALPHA_RESET_ACTION_DELTA="${ALPHA_RESET_ACTION_DELTA:-0.05}"
...
BASE=/workspace/matrice4d-pufferlib-v0/runpod_checkpoints/current_best/policy.bin \
BASE_LABEL=v1a_seed44_step170655744 \
SEEDS=44 \
  bash experiments/run_v1_dr_family_v1a2_mixed_edge.sh
EOF
  exit 2
fi
...
{
  echo "experiment=dr_family_v1a2_mixed_edge"
  ...
  echo "mix_profile_0=40pct_family_v05"
  echo "mix_profile_1=40pct_family_v1a_core_risk_le_2"
  echo "mix_profile_2=20pct_family_v1a_edge_targeted_risk_le_3"
  echo "edge_cases=low_authority,slow_motors,high_mass_inertia,hard_com"
...
run_seed() {
  local seed="$1"
  local run_name="v1_norm_thrust_dr_family_v1a2_mixed_edge_from_${BASE_LABEL}_seed${seed}_${TOTAL_TIMESTEPS}"
