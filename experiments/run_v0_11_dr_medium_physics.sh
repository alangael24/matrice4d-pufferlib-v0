...
BATCH_ID="${BATCH_ID:-v0_11_dr_medium_physics_scale07_$(date -u +%Y%m%dT%H%M%SZ)}"
...
# DR-medium physical parameters, intentionally without latency/noise.
...
BASE=/matrice4d-pufferlib-v0/base_dr_light_seed46.bin \\
  bash experiments/run_v0_11_dr_medium_physics.sh

This experiment is intended to continue from the V0.11 DR-light seed46 baseline.
EOF
  exit 1
fi
...

{
  ...
  echo "experiment=dr_medium_physics_no_latency_noise"
...
run_experiment() {
  local seed="$1"
  local run_name="v0_11_target5_dr_medium_physics_scale07_seed${seed}_${TOTAL_TIMESTEPS}"
...

  {
    ...
    echo "action_latency=$ACTION_LATENCY"
    echo "sensor_noise=$SENSOR_NOISE"
    echo "sweep_case=dr_medium_physics_scale07_no_latency_noise"
