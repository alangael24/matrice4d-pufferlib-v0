...
BATCH_ID="${BATCH_ID:-v0_11_dr_light_$(date -u +%Y%m%dT%H%M%SZ)}"
...
BASE=/path/to/v0_10_target5_seed46_scale0.5_yawMult5_vec4096x8x32_100m_latest.bin \\
  bash experiments/run_v0_11_dr_light.sh

Or extract/upload the v0_10 nominal target5 artifact so the script can find it.
EOF
  exit 1
fi
...
run_experiment() {
  local seed="$1"
  local run_name="v0_11_target5_dr_light_seed${seed}_${TOTAL_TIMESTEPS}"
