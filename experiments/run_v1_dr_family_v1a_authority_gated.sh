...
BATCH_ID="${BATCH_ID:-v1_norm_thrust_dr_family_v1a_authority_gated_$(date -u +%Y%m%dT%H%M%SZ)}"
BATCH_ROOT="${BATCH_ROOT:-runs/$BATCH_ID}"
BASE="${BASE:-}"
BASE_LABEL="${BASE_LABEL:-B2}"
SEEDS="${SEEDS:-42}"
...
BASE=/workspace/matrice4d-pufferlib-v0/base_B2_normalized_narrow_seed46.bin \
BASE_LABEL=B2_seed46 \
SEEDS=46 \
  bash experiments/run_v1_dr_family_v1a_authority_gated.sh
EOF
  exit 2
fi
...
{
  echo "experiment=dr_family_v1a_authority_gated"
...
run_seed() {
  local seed="$1"
  local run_name="v1_norm_thrust_dr_family_v1a_authority_gated_from_${BASE_LABEL}_seed${seed}_${TOTAL_TIMESTEPS}"
