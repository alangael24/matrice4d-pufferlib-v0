...
# Stage 1b: single-segment Swift-like racing with an Initial State Buffer.
#
# Starts from the successful Stage 1 gate-mask/no-privileged-critic checkpoint.
# Clean gate passes populate per-gate reset buffers. Resets then sample real
# post-gate states with perturbations, so the actor learns reacquisition instead
# of only geometric segment starts.
...
export ACTOR_OBS_MASK_TARGET=0.0
